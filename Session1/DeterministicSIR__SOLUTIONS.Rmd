---
title: 'Workshop 1. Basic dynamics: deterministic SIR - SOLUTIONS'
output:
  html_document:
    df_print: paged
...
seir <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    dS <- - beta*S*I
    dE <- beta*S*I - sigma*E  # sigma=1/latency_period
    dI <- sigma*E - gamma*I
    dR <- gamma*I
...
seir <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    dR <- gamma*I - mu*R
    dcumD <- mu*(S+E+I+R) + nu*I
    return(list(c(dS, dE, dI, dR, dcumD)))
  })
}

parameters <- c(beta=.8,sigma=.2,gamma=.1,mu=1/(70*356),nu=2e-3)
results <- ode(y=c(S=1-1e-5,E=.0,I=1e-5,R=.0,cumD=.0), times=seq(0,200*1,by=1), func=seir, parms=parameters)
...
plot_ly(df, x = ~df[,1], y = ~df[,2], name = 'S', type = 'scatter', mode = 'lines', color=I("blue")) %>%
  ...
  add_trace(y = ~df[,4], name = 'I', mode = 'lines', color=I("red")) %>%
  add_trace(y = ~df[,5], name = 'R', mode = 'lines', color=I("green")) %>%
  add_trace(y = ~df[,6], name = 'D', mode = 'lines', color=I("black")) %>%
  layout(xaxis = list(title = ''))
```
