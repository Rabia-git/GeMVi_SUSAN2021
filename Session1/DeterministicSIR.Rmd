---
title: 'Basic dynamics: deterministic SIR'
output:
  html_document:
    df_print: paged
...
NB. It's important to note that $R_0$ is a unitless measure, and that the rate of which the epidemic evolves cannot only be measured by $R_0$, it is also related to the generation time of the infection.
For instance, hiv can have an $R_0$ around 4, so that's 4 people infected by the same person during his/her infectiousness period (over a long period of time, a decade maybe). However, in case of measles, $R_0$ is between 15 to 18 and that is over just one week.
