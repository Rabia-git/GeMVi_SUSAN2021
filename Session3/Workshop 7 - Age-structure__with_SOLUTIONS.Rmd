...
for (i in 2:4){
  lines(times, prev_u20[,i]/N[i], col=i)
}

```

#### Age-wise contributions to the early exponential phase


It should be immediately apparent in the previous plot that prevalence varies by age class, and so the contribution made by each age class to the epidemic is not just proportional to the size of that age class. In fact, we can estimate these proportional contributions directly using the next generation matrix. 

Let us plot the total infections over time using a log y scale, and observe the early growth phase of the epidemic. We can use the next box for this:
...
You should find that there is an early exponential growth phase, during which the total infections grow linearly on this log scale. 
...
# SOLUTION
t_point = 20
prev_unscaled = results[t_point, I_cols] * (1/N)  # prevalence unscaled regarding the total number of infecteds, and this is calculated per age class and at time 20
prev_profile = prev_unscaled / sum(prev_unscaled) # we normalise it by dividing by the sum of all infecteds
barplot(unlist(prev_profile), xlab = "Age class", ylab = "Proportion")
```

We notice that infections are mostly concentrated in younger age classes, which is a common feature in epidemic modelling.
...
cat("With school closure R_0 in Kenya is reduced from about 2.5 to ",R0_ke_npi,".")
K_uk_npi = K_uk - K_uk_sc
R0_uk_npi = (tau / gam) * max(Re(eigen(K_uk_npi)$values))
cat("With school closure R_0 in UK is reduced from about 1.2 to ",R0_uk_npi,".")
```
