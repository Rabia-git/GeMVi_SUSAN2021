...
\begin{equation*}
\lambda_i(t) = \beta_i \sum_j \rho_{ij} \frac{I_j(t)}{N_i} = \frac{\beta_i}{N_i} \sum_j \rho_{ij} I_j(t)
\end{equation*}
...
\begin{equation*}
	\begin{array}{lcl}
	\displaystyle\frac{d S_i}{dt} & = & \displaystyle \mu (N_i - S_i(t)) - \lambda_i(t) S_i(t)\\ \mbox{}\\
	\displaystyle\frac{d I_i}{dt} & = & \displaystyle \lambda_i(t) S_i(t) - (\gamma+\mu)I_i(t)
	\end{array}
\end{equation*}


We now use the following notation: we denote elementwise multiplication with $\otimes$, and elementwise division with $\oslash$.  Using this notation, the force of infection takes the form:
\begin{equation*}
\vec{\lambda} = (\vec{\beta}\oslash \vec{N}) \otimes {\boldsymbol \rho} \cdot \vec{I}
\end{equation*}

\noindent
and it becomes straightforward to write down the model as:
\begin{equation*}
	\begin{array}{lcl}
	\displaystyle\frac{d \vec{S}}{dt} & = & \displaystyle \mu (\vec{N} - \vec{S}(t)) -
					\vec{\lambda}(t) \otimes \vec{S}(t) \\ \mbox{}\\
	\displaystyle\frac{d \vec{I}}{dt} & = & \displaystyle 
					\vec{\lambda}(t) \otimes \vec{S}(t) 
					- (\gamma+\mu) \vec{I}(t)
	\end{array}
	\qquad (2)
\end{equation*}
...
sir.model <- function( t, x, params )  {
	...
	lambda <- (beta/NN) * rho %*% pop.inf  # Matrix multiplication

	# Find rhs of (1) using vector calculus
	ds <- mu*(NN-pop.sus)-lambda*pop.sus
	di <- +lambda*pop.sus - (mu+gamma)*pop.inf
	res <- c(ds,di)
	return(list(res))
}
```
...
\caption{A representation of the country of Squareland: each subpopulation is located in space with a corresponding latitude and longitude. Note that we are storing the position as a (latitude,longitude) pair, where the latitude represents the vertical displacement and the longitude the horizontal displacement. \label{fig:squareland}}
\end{figure}
...
\noindent
First, we define a function that verifies whether two cells are nearest neighbours.  This is achieved by checking that the coordinates differ by a displacement corresponding to one of those implied by Fig \ref{fig:squareland-coupling}. The destination coordinates must be those of the source plus one of the four possible displacement.  Thus the difference between the coordinates of the destination and those of the source plus the displacement must be zero.  Since these are real numbers we will not use an exact comparison (though it would work in this case), and instead verify that the difference is sufficiently small.  The comparison is made elementwise by R, so we need to verify that both components of the resulting logical vector are TRUE, but this is easily achieved using the [sum] function.
```{r}
isNeighbour <- function( coors, src, dst )  {
	displ <- list(c(0.5,0.0), c(-0.5,0.0), c(0.0,0.5), c(0.0,-0.5))
	for (el in displ)  {
		if (sum(abs(coors[dst,] - (coors[src,] + el)) < 0.01) == 2)  {
			return(TRUE)
		}
	}
	return(FALSE)
...
Before leaving, we save the modified data sets. These may be used for exercises.
```{r}
if (!dir.exists("Data/Local"))  {
	dir.create("Data/Local")
}
counties.wgs84$ALAND <- NULL
counties.wgs84$AWATER <- NULL
